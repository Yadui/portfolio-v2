...
vec3 getColor(vec2 p) {
  ...
  float sum = digit(p + vec2(-off, -off)) + digit(p + vec2(0.0, -off)) + digit(p + vec2(off, -off)) + digit(p + vec2(-off, 0.0)) + digit(p + vec2(0.0, 0.0)) + digit(p + vec2(off, 0.0)) + digit(p + vec2(-off, off)) + digit(p + vec2(0.0, off)) + digit(p + vec2(off, off));
...
export default function FaultyTerminal({
  ...
  const gridX = Array.isArray(gridMul) ? gridMul[0] ?? 2 : 2;
  const gridY = Array.isArray(gridMul) ? gridMul[1] ?? 1 : 1;
  ...
  useEffect(() => {
    ...
    const enableMouse = mouseReact && supportsFinePointer;
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uUseMouse: { value: enableMouse ? 1 : 0 },
    ...
    const drawFrame = (timeStamp) => {
      ...
      const shouldAnimate = !pause && isIntersecting && isDocumentVisible;

      if (pageLoadAnimation && loadAnimationStartRef.current === 0 && shouldAnimate) {
    ...
    const syncAnimationState = () => {
      if (!pause && isIntersecting && isDocumentVisible) {
        startAnimation();
        return;
      }

      stopAnimation();
      drawFrame(lastTimeStamp || performance.now());
    };
